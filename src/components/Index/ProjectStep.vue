<template>
  <q-list separator>
    <q-item v-for="project in entries.projects" :key="project.id" clickable v-ripple
          :active="selection === project.id" active-class="bg-teal-1 text-orange-8" @click="selection=project.id">
  ...
  <AddProject v-model="prompt"></AddProject  oject>
...
//Methods
const onContinue = ()=>{
  entries.loadVisits(selection.value)
  emit('continue')
}
</script>
